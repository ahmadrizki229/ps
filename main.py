...
class SMSBower:
    ...
    def get_price(self):
        ...
        result = []
        country_map = {
            '16': {'name': 'United Kingdom', 'locale_psc': 'nl_NL', 'currency_psc': 'EUR', 'phone_code': '+44'},
        }
        try:
            response = self.r.get(self.base_url, params=params, proxies=proxies, timeout=10)
            response.raise_for_status()
...
def run(smsbower, password, smsbower_config):
    ...
    order_data = smsbower.buy_number(country_id='16', service_id='jq')
    if not order_data:
        return
...
def intro(smsbower_balance, capsolver_balance, price):
    ...
    return f"""BOT PSC UNITED KINGDOM
{b}{uper}
{b}{garis}{c}        {rocket} Informasi Pengguna {rocket}          {b}{garis}
{b}{garis}{c} {dot_blue} User              : RIZKI STORE
...
{b}{garis}{c} [1] United Kingdom  - stok {price[0]}
{b}{lower}{c}"""
...
def main():
    try:
        ...
        smsbower_config = {'country_code': '16', 'service': 'jq'}
